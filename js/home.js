...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (result) {
            let titles = Object.keys(result.data)
            let arr = []
            titles.forEach(t => {
                arr = arr.concat(result.data[t].map(item => {
                    item.title = t
    ...
    /*    $.ajax({
            ...
            contentType: "application/json;charset=utf-8",
            success: function(result) {
                let wrap = $('.navbar-notice')
                let str = result.data.map(item => {
                    return `<p>${item.title}:${item.content}</p>`
                }).join('')
                wrap.append(`<div class='notice-wrap'>${str}</div><div class='notice-wrap'>${str}</div>`)
                $('.notice-wrap').css('width', `${$('.notice-wrap').width() + 100}px`)
                wrap[0].style.setProperty('--contentWith', `-${$('.notice-wrap').width() + 100}px`)
            }
        });*/
    $.ajax({
        type: "get",
        url: baseUrl + "/nps/explain/list", //url
        dataType: "json",
        contentType: "application/json;charset=utf-8",
        success: function (result) {
            documentUrl = result.msg
        }
    });

    $.ajax({
        type: "get",
        url: baseUrl + "/nps/user/list", //url
        dataType: "json",
        contentType: "application/json;charset=utf-8",
        success: function (result) {
            var options = {
            ...
            var numAnim = new CountUp('ct', result.data-100, result.data, 0, 2.5, options);
            if (!numAnim.error) {
                numAnim.start();
            } else {
    ...
    if (localStorage.getItem('phone')) {
        $('.login-item').hide()
        $('.login-info-item').show()
        $('.user-info-item').text(localStorage.getItem('phone'))
    }
    if (localStorage.getItem('headUrl')) {
        $('.user-headUrl').attr('src', localStorage.getItem('headUrl'))
    }
    $('.logout-action-item').on('click', function () {
        localStorage.setItem('token', '')
        localStorage.setItem('phone', '')
        window.location.href = './login.html'
    })
    $('#commot').on('click', function () {
        window.open(documentUrl)
    })
    if (/Android|webOS|iPhone|iPod|BlackBerry/i.test(navigator.userAgent)) {
        $('.chat-qq').attr('href', 'http://wpa.qq.com/msgrd?v=3&uin=1163363720&site=qq&menu=yes')
    } else {
        $('.chat-qq').attr('href', 'tencent://message/?uin=1163363720&Menu=yes&Service=300&sigT=42a1e5347953b64c5ff3980f8a6e644d4b31456cb0b6ac6b27663a3c4dd0f4aa14a543b1716f9d45')
    }
})
